...
- 图片资源统一使用小写 `snake_case`，名称按
  `<功能>_<用途>_<状态或变体>.<扩展名>` 组织，例如
  `recipe_creation_manual.png`。
- 禁止在文件名中追加 `@2x`、`@3x` 等分辨率后缀；需要多分辨率时使用 Flutter
  约定的 `2.0x/`、`3.0x/` 目录变体。
- 资源归属明确：通用组件不内置业务图片；功能图片由所属 package 或根 App
  管理，并在对应的 `pubspec.yaml` 中声明。
